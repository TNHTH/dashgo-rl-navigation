...
def clear_runtime_state(
    ...
    *,
    ...
    pause_scope: str | None,
    resume_from: str | None,
    **extra: Any,
) -> dict[str, Any]:
    """收口运行态字段，避免 safe pause / stop 后残留假活动进程。"""
    return update_state(
        state_path,
        supervisor_status=supervisor_status,
        message=message,
        active_process_count=active_process_count,
        active_process_count_real=0,
...
def main(argv: list[str] | None = None) -> int:
    ...
    while not STOP_REQUESTED:
        ...
        if safe_pause_requested(state_path):
            ...
            already_paused = current_state.get("supervisor_status") == SUPERVISOR_STATUS_PAUSED
            clear_runtime_state(
                state_path,
                supervisor_status=SUPERVISOR_STATUS_PAUSED,
    ...
    clear_runtime_state(
        ...
        desired_state=load_state(state_path).get("desired_state"),
        pause_scope=load_state(state_path).get("pause_scope"),
        resume_from=str(paths["best_candidate"]),
        last_heartbeat_at=iso_now_local(),
    )
