...
STABLE_CHECKPOINT="$PROJECT_ROOT/autopilot/runs/gen2/20260319_024028_wave44_gen2_model655_stablehistory_seed44_capture/checkpoints/model_704.pt"
LATEST_CHECKPOINT="$PROJECT_ROOT/autopilot/runs/gen2/20260319_024319_wave45_gen2_model704_stablehistory_extend375/checkpoints/model_743.pt"
...
case "$mode" in
  play)
    ...
    cmd=(
      "$ISAACLAB_SH" -p "$PROJECT_ROOT/play.py"
      --num_envs "$num_envs"
  ...
  sensor)
    ...
    cmd=(
      "$ISAACLAB_SH" -p "$PROJECT_ROOT/inspect_live_env.py"
      --headless
      --profile "$profile"
      --num_envs "$num_envs"
      --steps "$steps"
  ...
  fullstack)
    ...
    echo "profile: $profile"
    echo
    cmd=("$ISAACLAB_SH" -p "$PROJECT_ROOT/verify_ultimate_v5.py")
    cmd+=("${extra_args[@]}")
    exec "${cmd[@]}"
    ;;
esac
