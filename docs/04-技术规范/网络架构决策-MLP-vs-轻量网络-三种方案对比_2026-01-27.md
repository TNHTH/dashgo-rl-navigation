...
# 2. 启动训练
~/IsaacLab/isaaclab.sh -p apps/isaac/train_v2.py --headless --enable_cameras --num_envs 64
...
# 2. 上传到Jetson
scp policy_v2.pt jetson@dashgo:~/ros1_catkin_ws/src/dashgo_navigation/
...
# 2. 等待8000轮训练完成
# 3. 保存教师模型: .artifacts/train/logs/dashgo_v5_auto/model_8000.pt
```
...
def collect_data():
    ...
    teacher_model = load_model(".artifacts/train/logs/dashgo_v5_auto/model_8000.pt")
...
**预期结果**：
- 训练正常进行
- 生成MLP模型（.artifacts/train/logs/dashgo_v5_auto/model_8000.pt）
- **但无法直接部署到实物**
...
**文档维护者**: TNHTH (Robot-Nav-Architect Agent)
**等待决策**: 方案选择（1/2/3）
**优先级**: 🔴 高（阻塞训练启动）
