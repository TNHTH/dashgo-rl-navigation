...
| Action | normalized `[v,w]`, bounded tanh Gaussian; platform capability is 0.3 m/s forward, 0.15 m/s reverse and 1 rad/s yaw; formal forward-sensor execution envelope has `v>=0` | policy, platform sources and experiment preregistration | capability and effective command envelope are separately hashed; commanded velocity is not measured motion |
...
- `without_shield` disables both shield bias and its losses/diagnostics-as-interventions; `without_lreg` disables only Lreg; `without_acsi` disables the complete ACSI chain: collision capture/reservation, selection, curriculum/level update, and replay/reset. Ordinary collision termination stays common. Derived switches must be enumerated so profiles cannot silently change reward, normalization or sampling behavior.
- `ablation_profile=full|without_*` and the existing `runtime_profile=gen1|gen2|autopilot` are independent manifest axes. Formal comparisons keep the runtime axis identical.
- D1 preregisters `lookahead_distance_m=0.20` and simulated `max_sensor_age_s=0.10` inclusive as versioned cross-platform engineering choices. D1 hashes only sensor-source configuration; D2 owns actual 216-ray geometry and the final raw-safety hash.
