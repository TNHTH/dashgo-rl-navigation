...
1. **Git recovery and baseline** — complete: four old heads have immutable remote archive tags; remote branches now exactly `main/stable/test@ad9cf5a`; baseline CPU suite is 43 passed.
2. **SEA public core dependency** — wait for an independently accepted `sea_nav_core` commit and pin its full SHA.
3. **DashGo contracts and policy** — add the separate safety observation group, action codec/trace, alpha head, pure auxiliary queries and CBF mean-stage integration without changing sampled-action/log-prob identity.
...
|---|---|---|---|---|---|
| primary `work/dashgo-rl-navigation` | `test@ad9cf5a1b021687862a8abb9a9ab315ca54e72c2` | controller | coordination, integration and final verification | recovery tags | active |
| `work/dashgo-rl-navigation-sea-adapter` | detached from `ad9cf5a…` | `/root/review_batch7_core` | `src/dashgo_rl/sea_nav/**`, focused tests, experiment config/tooling, and only the necessary training/env/export/eval callers | accepted SEA core API | registered; implementation waits for API commit |
